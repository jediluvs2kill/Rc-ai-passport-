...
import { CheckCircle2, AlertCircle, Play, Image as ImageIcon, Zap, Settings } from 'lucide-react';
          ...
          >
            ...
            <div className="flex justify-between items-center mt-2">
                ...
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border uppercase ${
                    car.class.includes('Stock') 
                    ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' 
                    : 'bg-orange-500/10 text-orange-400 border-orange-500/20'
                }`}>
                    {car.class.includes('Stock') ? 'Stock' : 'Modified'}
                </span>
            </div>
          </div>
        ))}
      ...
      <div className="flex-1 flex justify-center overflow-y-auto">
        ...
        <div className="w-full max-w-md bg-slate-900 border border-slate-700 rounded-[2.5rem] p-6 md:p-8 shadow-2xl h-fit">
            ...
            <div className="mb-6 flex justify-between items-start">
                ...
                {/* Visual Toggle for MVP */}
                <div className="flex bg-slate-950 rounded-full p-1 border border-slate-800">
                    <div className={`px-3 py-1 rounded-full text-xs font-bold transition-colors ${selectedCar.class.includes('Stock') ? 'bg-emerald-600 text-white' : 'text-slate-500'}`}>
                        Stock
                    </div>
                    <div className={`px-3 py-1 rounded-full text-xs font-bold transition-colors ${!selectedCar.class.includes('Stock') ? 'bg-orange-600 text-white' : 'text-slate-500'}`}>
                        Modified
                    </div>
                </div>
            </div>
            ...
            {/* Car Image */}
            <div className="mb-6 rounded-2xl overflow-hidden border border-slate-700 bg-slate-800 aspect-video relative group">
                <img src={selectedCar.imageUrl} alt={selectedCar.nickname} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors"></div>
                <div className="absolute bottom-2 right-2 bg-black/60 backdrop-blur px-2 py-1 rounded text-[10px] text-white font-mono">
                    ID: {selectedCar.id.toUpperCase()}
                </div>
            </div>

            {/* Vehicle Details Card (MVP Style) */}
            <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 mb-6">
                 <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Vehicle Details</h3>
                 <div className="grid grid-cols-2 gap-4">
                     <div>
                         <label className="text-[10px] text-slate-500 uppercase">Chassis Make</label>
                         <div className="text-white font-bold text-sm">{selectedCar.chassis.split(' ')[0]}</div>
                     </div>
                     <div>
                         <label className="text-[10px] text-slate-500 uppercase">Model</label>
                         <div className="text-white font-bold text-sm">{selectedCar.chassis.split(' ').slice(1).join(' ') || 'Pro'}</div>
                     </div>
                     <div>
                     ...
                     <div>
                         ...
                         <div className={`font-bold text-sm ${selectedCar.verified ? 'text-emerald-400' : 'text-yellow-400'}`}>
                             {selectedCar.weight}g {selectedCar.verified ? '✓' : '(Pending)'}
                         </div>
                     </div>
                 </div>
            </div>
            ...
            <div className="bg-slate-950/50 rounded-2xl border border-slate-800 overflow-hidden mb-8">
                ...
                <div className="flex justify-between items-center p-4 border-b border-slate-800/50">
                    <span className="text-slate-400 font-medium text-sm flex items-center gap-2"><Zap size={14}/> Motor</span>
                 ...
                 {/* ESC */}
                 <div className="flex justify-between items-center p-4 border-b border-slate-800/50">
                    <span className="text-slate-400 font-medium text-sm flex items-center gap-2"><Settings size={14}/> ESC</span>
                    <span className="text-white font-bold text-sm">
                        {selectedCar.parts.find(p => p.type === 'ESC')?.brand || 'Generic'}
                    </span>
                </div>
                {/* Battery */}
                 <div className="flex justify-between items-center p-4">
                    <span className="text-slate-400 font-medium text-sm flex items-center gap-2"><Zap size={14}/> Battery</span>
                    <span className="text-white font-bold text-sm">
                        {selectedCar.parts.find(p => p.type === 'Battery')?.spec || 'Unknown'}
                    </span>
                </div>
            </div>
            ...
            {/* Proof Timeline */}
            <div>
                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Verification Media</h3>
